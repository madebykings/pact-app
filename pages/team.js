...
export default function Team() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Create settings row only if missing (DO NOT overwrite mode)
        {
  ...
  async function setMode(mode) {
    ...
    // Read team_id fresh to avoid stale settings
    const { data: st, error: stErr } = await supabase
      .from("user_settings")
      .select("team_id")
      .eq("user_id", user.id)
  ...
  async function invite() {
    ...
    // Try status 'pending', fallback to DB default if constraint differs
    let error = null;

    {
      const res = await supabase.from("team_invites").insert({
    ...
    if (error) {
      const res2 = await supabase.from("team_invites").insert({
        team_id: settings.team_id,
        email,
        token,
        created_by: user.id,
      ...
      {!isInTeam ? (
        ...
        <>
          ...
          <div style={{ marginTop: 14, padding: 14, border: "1px solid #ddd", borderRadius: 12, background: "#fff" }}>
            ...
            <div style={{ marginTop: 10, fontSize: 13, opacity: 0.7 }}>Invites generate a link the person can open after logging in.</div>
          </div>
